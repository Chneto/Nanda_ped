...
function renderForecast12MSVG(projectionData, options = {}) {
  ...
  const gridLines = gridSteps.map(step => {
    ...
    return `
      <line x1="${padLeft}" y1="${y}" x2="${width - padRight}" y2="${y}" stroke="currentColor" stroke-opacity="0.08" stroke-dasharray="3 3" />
      <text x="${padLeft - 6}" y="${y + 3}" fill="currentColor" fill-opacity="0.45" font-size="9" font-family="Manrope" text-anchor="end">${valK}k</text>
    `;
  }).join("");
  ...
  const barsMarkup = points.map((pt, i) => {
    ...
    return `
      <g class="scrubber-column group cursor-pointer"
        ...
        <!-- Bar background track -->
        <rect x="${barX}" y="${padTop}" width="${barWidth}" height="${chartHeight}" rx="10" fill="#ebedff" fill-opacity="0.35" />
        ...
        <!-- Value Label -->
        <text x="${pt.x}" y="${Math.max(pt.yBar - 6, 14)}" fill="${isCurrent ? '#EC407A' : '#7e4a8a'}" font-size="9" font-family="Manrope" font-weight="700" text-anchor="middle">
          ${inflowK}
        </text>

        <!-- X Axis Month Label -->
        <text x="${pt.x}" y="${height - 10}" fill="${isCurrent ? '#EC407A' : '#4d444d'}" font-size="10" font-family="Manrope" font-weight="${isCurrent ? '700' : '500'}" text-anchor="middle">
          ${pt.data.label}
        </text>
      </g>
    `;
  ...
  // Balance Dots
  const dotsMarkup = points.map(pt => `
    <circle cx="${pt.x}" cy="${pt.yBalance}" r="3.5" fill="#ffffff" stroke="#7e4a8a" stroke-width="2" class="pointer-events-none" />
  `).join("");
