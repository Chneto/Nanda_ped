 ...
 */
export function spawnPediatricParticles(x = null, y = null, count = 8, type = 'mixed') {
  if (typeof document === 'undefined') return;
 ...
 */
export function getDoctorPhoto() {
...
export function setDoctorPhoto(photoBase64) {
  if (photoBase64) {
...
export function handleDoctorPhotoFileSelect(file) {
  if (!file) return;
 ...
 */
export function openHamburgerDrawer() {
...
export function closeHamburgerDrawer() {
 ...
 */
export function openLegalComplianceModal() {
  const html = `
 ...
 */
export function renderCustomSelectHTML({ id, name, value, options = [], icon = null, extraClass = "" } = {}) {
  let selectedLabel = "";
  ...
  return `
    <div class="custom-select-container ${extraClass}" data-select-id="${id}">
      ...
      <div class="custom-select-popover hidden" id="${id}-popover" role="listbox">
        <div class="custom-select-list no-scrollbar">
          ${normalizedOptions.map(opt => {
 ...
 */
export function attachCustomSelectEvents(root = document) {
...
// Global click-outside listener for custom selects
