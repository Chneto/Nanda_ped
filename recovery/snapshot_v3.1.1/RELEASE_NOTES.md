...
> **Versão Oficial Atual: v3.1.1 (Anti-Crop Smart Select & Modal Scroll Safeguard)**  
> **Criado por: FChNeto**  
...
## 🌟 Versão 3.1.1 (Anti-Crop Smart Select & Modal Scroll Safeguard)

A versão **3.1.1** corrige de forma cirúrgica e abrangente o bug de corte visual (*crop*) nos menus suspensos e garante rolagem interna fluida em todos os modais da aplicação:

### 🚀 Correções & Melhorias da Versão 3.1.1

#### 1. 🛡️ Correção Definitiva do Corte no Dropdown de Alíquotas e Seletores
- **Expansão em Fluxo Normal (In-Flow Expansion) em Modais:** Em cartões de diálogo (`.dialog-card` e `.dialog-box`), o menu suspenso `.custom-select-popover` expande no fluxo normal do documento (`position: relative !important`). Isso empurra os botões inferiores ("Voltar" e "Próximo") naturalmente para baixo, expande a altura de rolagem do cartão (`scrollHeight`) e elimina completamente qualquer corte pela borda arredondada do modal.
- **Scroll Automático & Sem Bloqueio de Toque:** A lista interna adota `overscroll-behavior: auto`, permitindo encadeamento natural de gestos de toque (*touch momentum*). Ao abrir, o modal faz rolagem suave para garantir que o menu e os botões de ação permaneçam 100% visíveis.
- **Eliminação de Conflitos de Rolagem Aninhada:** O `.dialog-card-content` foi otimizado com espaçamento inferior generoso (`pb-8`), eliminando contêineres de rolagem redundantes e entregando rolagem perfeita no Safari iOS e Chrome Android.
- **Detecção Inteligente de Proximidade & Abertura Superior (Dropup):** Fora de modais (ex.: barras de ferramentas e bottom-sheets), os seletores mantêm abertura flutuante com dropup inteligente quando próximos do rodapé da tela.
- **Fechamento com Tecla Escape & Toque Externo:** Suporte para fechar menus ao pressionar `Esc` ou tocar fora do componente.

#### 2. 🏥 Opções Tributárias Médicas Completas no Onboarding
- O passo 2 do assistente de Onboarding ("Seus Hospitais & Regras 🏥") agora inclui toda a gama realista de regimes tributários da pediatra:
  - `PJ Simples Nacional (6% com Fator R)`
  - `PJ Lucro Presumido (~11.33%)`
  - `PJ Lucro Presumido (~13.33%)`
  - `PJ Simples Anexo V (15.5%)`
  - `PJ Lucro Presumido (~16.33%)`
  - `Carnê-Leão PF / RPA (27.5%)`

#### 3. 🧪 Suíte de Testes Expandida para 85 Testes Aprovados
- Arquivo de teste `tests/select_crop_fix.test.js` cobrindo rigorosamente as proteções anti-crop, expansão in-flow em modais, detecção inteligente de diálogos, ausência de overflow:hidden impeditivo, compatibilidade dark mode e sincronização do bundle.
